...
### 新增
- LLM 模型選擇器：可在設定中自由切換 LLM 模型（支援 GPT-4o / GPT-4.1 / GPT-5 系列等）
- 切換模型後即時生效，無需重啟應用程式
- MenuBar 即時顯示目前使用的 LLM 模型名稱

### 變更
- 預設 LLM 模型從 gpt-4o-mini 升級為 gpt-4.1
- LLM temperature 從 0.7 降低為 0.1，提升文字整理的一致性

> 為求最佳效果，建議使用 GPT-4.1 模型。

### 修正
- 強化 LLM 系統提示詞，新增問句範例防止 LLM 搶答問題而非整理文字
- 新增設定遷移 v2→v3（提示詞強化）及 v3→v4（模型選擇欄位）
